...
Where `Π n. i_n -> m j_n` is mathematician's way of saying `forall n. i n -> m (j n)`. Up to a very small difference (rename `i` to `f`
...
thought of as a consequence of the fact that in `forall a. f a -> m a`, `f` cannot refer to `m`.
...
move it “left of the fat arrow”: really in Haskell `A -> B` and `A => B` mean the same thing, they only differ in whose responsibility it is
...
three approaches][torreborre-capabilities-as-records]: _capabilities
