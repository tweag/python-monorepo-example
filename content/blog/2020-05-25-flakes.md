...
[*flakes*](https://github.com/NixOS/rfcs/pull/49), a new Nix feature
...
So the `dwarffs` flake depends on a *specific* version of the
...
version in `dwarffs`'s `flake.nix`. Instead, it's recorded in a *lock
file* named
...
functions. These are called its *outputs*. We can see the outputs of a
flake as follows:
...
* The `outputs` attribute is the heart of the flake: it's a function
  ...
  The `self` argument denotes *this* flake. Its primarily useful for
