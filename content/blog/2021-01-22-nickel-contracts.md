...
`blame` signals rejection with an optional error message attached via `tag`.
When evaluating `value | #Contract`, the interpreter calls `Contract` with an
appropriate label and `value` as arguments.
...
contracts. The`#` character distinguishes _contracts as types_ from _contracts
as functions_ (that is, custom contracts).

To sum up, contracts are just glorified assertions. Also, there is this
incredibly convenient syntax that spares us a whole three characters by writing `Num`
instead of `#MyNum`. So... is that all the fuss is about?
...
Nickel is a [lazy programming language][laziness]. This means that expressions,
...
not seem to trigger, you can use the `deepSeq` operator to recursively force the
