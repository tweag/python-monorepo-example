...
So why build a new one? It mostly came down to a few things: 
- Integration 
- Caching
...
Using arrow syntax also gives us some additional advantages. Unlike traditional
do notation, arrows _must_ have all their arguments passed in explicitly - they
cannot close over arbitrary variables in their environment. This means that we
cannot end accidentally pulling in unneeded dependencies to our results. And
being a Haskell DSL means we get all the benefits of typing to prevent composing
invalid workflows.
...
adapting Bazel to build polyglot Haskell projects. So one might be tempted to
ask - why not just use one of these?

One certainly could build such a workflow using nix or bazel, and this wouldn't
be a bad option. But there are a few reasons not to. Firstly are the prosaic -
nix doesn't support the same notion of content addressable storage as does
Funflow (although [it might be getting
it](https://github.com/edolstra/nix/commit/236e87c687065c2aa935542533b1a65282a73347)).
Bazel lacks the precise control over the control flow that we want, as well as
the typing.
...
it. In Funflow, one builds a series of steps with inputs and outputs, and then
wire them together to create a pipeline. Each step has no explicit knowledge of
the steps before or after it. It's like the difference between successive
function application to a value and functional composition.
