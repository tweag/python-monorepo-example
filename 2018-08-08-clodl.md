...
any compiler. But there are tools that follow this approach as well.

There is a commercial tool called [ermine][ermine], which produces a
self-contained executable from a dynamically-linked application.
Unfortunately, it provides no clues on the structure of the final
executable. `clodl`, on the contrary, produces a zip archive that can be
manipulated by client applications. We can't depart with this ability if
we are to load the contents in the JVM.
...
However, `exodus` wouldn't work for building closures of libraries.
