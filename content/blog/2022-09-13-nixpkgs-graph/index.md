...
[Nix](https://nixos.org/) is a package manager with a focus on reproducible, declarative and reliable packages.
...
This rigor-by-design with respect to dependencies is what makes Nix packages highly reproducible, and as a side effect, it gives us a fantastic dataset: the full dependency network that the more than 80000 packages in this collection form.
...
In contrast to an [earlier blog post](https://www.tweag.io/blog/2019-02-06-mapping-open-source/) that extracted the graph in an adhoc way directly from Nix's database of packages, the main purpose of this project is to provide a CLI tool that simplifies the extraction of derivations (Nix's name for packages) and their dependencies programmatically from nixpkgs, and injecting them as nodes and edges in a graph database for further examination.

I thus sincerely encourage you to visit the [GitHub repository](https://github.com/tweag/nixpkgs-graph/) after reading this post. Now let's get into it!
...
In the context of nixpkgs, we can interpret nodes as derivations (packages) identified by their name, in the form of `pname-version`.
...
The `--eval` flag will allow the `nix-instantiate` command to evaluate the Nix expressions without instantiations of store derivations taking place.
And this is just what we need.

Then, for each attribute under nixpkgs, we first check whether it is a derivation using the Nix lib function `isDerivation`.
...
The CLI of this project allows the user to customize the version of nixpkgs to be used.
...
Using the gexf format file provided by default after running with the CLI, we can draw the following image with Gephi.
As shown below, we set the color of the nodes according to the group they belong to.
And the size of each node is nonlinearly and positively related to its [in degree](https://en.wikipedia.org/wiki/Directed_graph) (def: the number of edges coming into a node in a directed graph).

For the layout of the graph, the [atlas force 2](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0098679) algorithm is used here.
...
But since our id identification uses `pname-version`, they are identified as one node, thus forming a cycle.
...
To take this a step further, we can compare the differences between python39 and Python310 replacing the 'python' string by 'python39' and 'python310' in the previous query.
The results show that python's references to gnuradio are mainly a Python 3.10 thing.

The above is just a preliminary use of Neo4j for the nixpkg graph.
...
The graph of nixpkgs on the one hand allows us to visualize through gephi and thus visualize the interactions between different software ecosystems. On the other hand, it allows us to perform precise queries through Neo4j.
...
This [project](https://github.com/tweag/nixpkgs-graph/) provides users with raw materials and some tools that they can explore according to their needs to explore.
