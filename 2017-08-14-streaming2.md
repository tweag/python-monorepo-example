...
as they allow programs to be conveniently composed.
...
ways. Let us first discuss streaming libraries that
manipulate stream processors. Generally, a stream processor
...
`p` until it completes, and then passed to `q` to produce the remainder.
As a result, the output stream of `p` is followed by the output stream
of `q`.

In contrast to `pipes` and `conduit`, the packages
[streaming](http://hackage.haskell.org/package/streaming)
and
[io-stream](http://hackage.haskell.org/package/io-streams)
manipulate streams directly.
Consequently, there is no notion of an *input stream*. Still, monadic
composition works in a similar manner in the case of the package `streaming`. The
...
The second form of composition, namely pipelining, is realised by the following to combinators in the libraries based on 
stream processors:
...
In `p .| q`, the output stream of `p` is fed into `q` as its input stream. 
In the packages `streaming` and `io-streams`, pipelines are constructed
...
Turning out attention to the packages directly manipulating streams, `Stream`s from the package `streaming` provide a function `cons` to push values back
into a stream. The package `io-streams` provides `unRead`, for the same purpose.
While `cons` function just constructs a new stream, the
function `unRead` modifies the input stream. We have,
...
list as they are discovered. The list is handed back to the caller only
...
After all, they provide a crucial resource usage guarantee that we cannot obtain otherwise.
...
With `conduit`s and `pipes` one can break and fix the program in a
similar way. Thus, streaming libraries seem to require that the programmer takes care to only
...
patterns. Leftovers, for instance, were support parsing
challenges that cannot be easily delegated to standard parsing libraries while still keeping
an upper bound on the consumed memory. Moreover, `ZipSink`s simplify the sharing of a single input stream
