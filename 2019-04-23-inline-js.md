---
title: "Inline-js"
shortTitle: "Inline-js"
author: Shao Cheng
tags: haskell, javascript, nodejs
---

Among the efforts to bring more functionalities to the Haskell ecosystem, a
common approach is to implement bridges to other languages, making popular
foreign libraries easily callable in Haskell. Along this way, there has already
formed a family of `inline-*` packages, all of which providing utilities to
allow embedding foreign code snippets directly in Haskell sources, therefore
reducing the amount of work required to implement library bindings (or even
invalidating the need to write one!) Now, we're glad to announce a new member in
this family: [`inline-js`][inline-js], which smoothens Haskell/Node.JS interop
experience and adds a huge ecosystem to our toolkit.
...
`block` quotes a series of JavaScript statements to run, and in-scope Haskell
...
is a `FromJSON` instance, so that instance is picked up, and on the JavaScript
...
since it works out-of-the-box on both web and node, so is more future-proof.
...
getMagnet magnet filename =
  withSystemTempDirectory "" $ \tmpdir -> do
    withCurrentDirectory tmpdir $ callCommand "npm install webtorrent"
    withJSSession
...
possible to work with multiple installations of `node` without recompiling. The
schema of binary messages and implementation of "eval server" is hidden from
users, thus can evolve without breaking exposed API of `inline-js`.
...
code, while still adding custom classes or functions useful to the eval server.
...
accompanied by threads which perform actual sending/receiving. All user-facing
...
All captured variables should be a `ToJSON` instance, serialized to JSON which
is also a valid piece of ECMAScript, and whatever returned value should also be
a `FromJSON` instance. However, there are annoying exceptions which aren't
appropriate to get `FromJSON`/`ToJSON` instances.
...
"post-processing" JavaScript function to encode the result to binary at the
`node` side, another to deserialize from binary at the Haskell side. For the
...
## Passing references of arbitrary JavaScript values

It's also possible to pass opaque references of arbitrary JavaScript values
...
caution. We also lose the reentrancy with this "sync mode"; when the exported
...
We've now presented demos on how `inline-js` allows quoting JavaScript code to
be directly used in Haskell, and explained several key aspects of `inline-js`
internals. The core ideas are quite simple, and there are endless potential use
cases, given the enormous ecosystem the Node.JS community has accumulated over
the past few years. Even for development tasks not specifically tied to Node.JS,
it would still be nice to have the ability to easily call relevant JavaScript
libraries, to accelerate prototyping in Haskell and to compare
correctness/performance of Haskell/JavaScript implementations.

There are still potential improvements to make, e.g. implementing type-based
exporting of Haskell functions. But we decide it's good timing to announce it
