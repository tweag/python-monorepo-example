...
Iso-deriving inserts `inj` and `proj` in the definition of type class
methods. The performance of iso-derived instances depends on the
performance of `inj` and `proj`. Sometimes, GHC will optimise `inj`
and `proj` away, but you shouldn't count on it.

If performance is an issue, you want your instances to be hand-written
instead.
