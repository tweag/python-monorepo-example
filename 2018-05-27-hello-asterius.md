...
Most importantly, we'd like to take a *progressive* approach at bringing Haskell to WebAssembly. The GHC runtime system is written with a traditional operating system environment in mind, making use of the C runtime, file systems, threads and all that. While projects like [Emscripten](https://github.com/kripken/emscripten) try to implement some if not all of those functionalities, porting a complete runtime along with its dependent libraries is surely a non-trivial challenge. However, to get a simple example like fibonacci run to completion, the portion of `rts` required to port to WebAssembly is quite small!

Once trivial examples like fibonacci gets to run, we gain confidence that we've set foot on a reasonable path. We can then iteratively write more complex examples, check what's missing and implement the missing bits. If some critical flaw arises, we get feedback early.
...
Currently `asterius` does not support compiling C. However, being able to compile simple C files in `cbits` of common packages (like `bytestring`) shall be a big win, since they often have little additional dependency other than the C runtime, and compiling from C to WebAssembly is relatively well-tested by projects like Emscripten.

If we bring in Clang/LLVM, we'll need to modify our codegen to suit LLVM's calling convention, and switch away from our own linker and object store format. In return, we don't need to write a WebAssembly shim every time we encounter a dependency on foreign C function, and it may even be possible to cross-compile a pruned version of `rts` itself, reducing the workload of implementing the `asterius` runtime.
...
* Easing the maintainance of custom GHC patches. Currently we maintain a `ghc-toolkit` package which works by pasting and patching quite some code from the GHC tree, but integrating upstream changes is troublesome. There may even be patches that render the normal ELF object files corrupted, but we don't need those object files anyway, they're just by-products of `asterius` WebAssembly compilation pipeline.

Not using GHC API provided by the host GHC may cause problems, e.g. losing the ability to run Template Haskell. But compared to the benefits above, it's still a win.
